...
// long strip 0,1,2.. white cells
//  control -> pointer at 0 and shift
//  pointer x to x+1 , hold shift , release -> all visited during shift hold turn black
//  color at least k cells but inside segment mentioned
...
void solve()
{
    ...
    for (int i = 1; i <= n; ++i)
        cin >> R[i];
    if (accumulate(all(R), 0ll) - accumulate(all(L), 0ll) + n < k)
    {
        cout << -1 << '\n';
        return;
    }
    ...
    for (int i = 1; i <= n; ++i)
    {
        if (D - x < k)
        {
            if (D + (R[i] - L[i] + 1) >= k)
            {
                ans = min(ans, L[i] + 2 * i - 2 + k - D - min(x, D - k + R[i] - L[i] + 1));
            }
        }
        D += (R[i] - L[i] + 1);
        x += (L[i] == R[i]);
